...
import type { ToolMode, Annotation, HomePosition } from "../components/glb/types";
...
import { NewAnnotationDialog, EditAnnotationDialog } from "../components/glb/AnnotationDialog";
...
export default function GlbViewer() {
  ...
  const [pendingAnnotation, setPendingAnnotation] = useState<{ world: THREE.Vector3; screen: { x: number; y: number } } | null>(null);
  const pendingAnnotationRef = useRef<{ world: THREE.Vector3; screen: { x: number; y: number } } | null>(null);
  ...
  useEffect(() => { pendingAnnotationRef.current = pendingAnnotation; }, [pendingAnnotation]);
  ...
  const perimeter = measurePoints.length >= 3 ? polygonPerimeter(measurePoints) : null;
  ...
  const drawOverlay = useCallback(() => {
    ...
    const drawDot = (pt: THREE.Vector3, color: string, r = 6) => {
      ...
      ctx.beginPath(); ctx.arc(p[0], p[1], r + 1.5, 0, Math.PI * 2);
    ...
    const drawLabel = (pt: THREE.Vector3, text: string, color: string) => {
      const p = project3Dto2D(pt, camera, W, H);
      if (!p) return;
      const px = p[0] + 10, py = p[1] - 10;
      ctx.font = "bold 12px system-ui,sans-serif";
      const tw = ctx.measureText(text).width;
      ctx.fillStyle = "rgba(0,0,0,0.85)";
      roundRect(ctx, px - 4, py - 13, tw + 18, 22, 5); ctx.fill();
    ...
    if (mode !== "none" && pts.length > 0) {
      ...
      if (mode === "area" && pts.length >= 3) drawLine([pts[pts.length - 1], pts[0]], "#60a5fa", true);
      pts.forEach((pt, i) => drawDot(pt, i === 0 ? "#10b981" : "#ffd700"));
      // Segment labels
      for (let i = 0; i < pts.length - 1; i++) {
        drawLabel(pts[i].clone().lerp(pts[i + 1], 0.5), fmtLen(pts[i].distanceTo(pts[i + 1]), unit), "#ffd700");
      }
      if (mode === "area" && pts.length >= 3) {
        drawLabel(pts[pts.length - 1].clone().lerp(pts[0], 0.5), fmtLen(pts[pts.length - 1].distanceTo(pts[0]), unit), "#ffd700");
      }
      // Hover preview
      ...
      if (hover) {
        ...
        drawDot(hover, "#fff", 4);
        drawLabel(hover, fmtLen(pts[pts.length - 1].distanceTo(hover), unit), "#93c5fd");
      }
    } else if (mode !== "none") {
      const hover = hoverRef.current;
  ...
  useEffect(() => {
    ...
    api.get(`/schools/${id}`).then(res => {
      ...
      const building = s.buildings?.find((b: any) => b.modelPath?.toLowerCase().endsWith(".glb"));
    ...
    }).catch(() => { setError("Failed to load school data."); setLoading(false); });
  ...
  function initViewer(glbUrl: string, savedHome: HomePosition | null) {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        if (footprint > 0) { model.scale.setScalar(80 / footprint); model.updateMatrixWorld(true); }
      ...
      err => { console.error(err); setError("Failed to load 3D model."); setLoading(false); },
    );
    ...
    const onResize = () => {
      ...
      if (overlay) { overlay.width = nW; overlay.height = nH; }
    };
  ...
  function getModelHit(clientX: number, clientY: number) {
    const model = modelRef.current, camera = cameraRef.current, renderer = rendererRef.current;
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    const mode = toolModeRef.current;
    if (mode === "none" || mode === "annotate") { hoverRef.current = null; return; }
  ...
  const handleClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (mode === "distance") {
      setMeasurePoints(prev => prev.length >= 2 ? [hit.point.clone()] : [...prev, hit.point.clone()]);
    } else if (mode === "area") {
      setMeasurePoints(prev => [...prev, hit.point.clone()]);
    } else if (mode === "annotate") {
      const renderer = rendererRef.current!;
      const rect = renderer.domElement.getBoundingClientRect();
      setPendingAnnotation({ world: hit.point.clone(), screen: { x: e.clientX - rect.left, y: e.clientY - rect.top } });
      setAnnotationLabel("");
    }
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const saveAnnotations = useCallback(async (updated: Annotation[]) => {
    ...
    try { await api.patch(`/schools/${id}`, { glb3dAnnotations: updated }); }
  ...
  const confirmAnnotation = useCallback(() => {
    ...
    const ann: Annotation = {
      id: uid(), label: annotationLabel.trim(), color: annotationColor,
      position: { x: pendingAnnotation.world.x, y: pendingAnnotation.world.y, z: pendingAnnotation.world.z },
      createdAt: new Date().toISOString(),
    };
    const updated = [...annotationsRef.current, ann];
    setAnnotations(updated); saveAnnotations(updated);
    setPendingAnnotation(null); setAnnotationLabel("");
  ...
  const confirmEdit = useCallback(() => {
    ...
    const updated = annotationsRef.current.map(a => a.id === editingAnnotation ? { ...a, label: editLabel.trim() } : a);
    setAnnotations(updated); saveAnnotations(updated);
    setEditingAnnotation(null); setEditLabel("");
  ...
  const saveHomePosition = useCallback(async () => {
    ...
    try {
      ...
      setHomePosition(home); setHomeSaved(true);
  ...
  const goHome = useCallback((home: HomePosition) => {
    ...
    const cam = camera, ctrl = controls;
    const startPos = cam.position.clone(), startTarget = ctrl.target.clone();
    const endPos = new THREE.Vector3(home.position.x, home.position.y, home.position.z);
    const endTarget = new THREE.Vector3(home.target.x, home.target.y, home.target.z);
    let t = 0;
    const clock = new THREE.Clock();
    function tick() {
      t += clock.getDelta() / 1.2;
      if (t >= 1) { cam.position.copy(endPos); ctrl.target.copy(endTarget); ctrl.update(); rendererRef.current?.render(sceneRef.current!, cam); return; }
      const ease = t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t;
      cam.position.lerpVectors(startPos, endPos, ease);
      ctrl.target.lerpVectors(startTarget, endTarget, ease);
      ctrl.update(); rendererRef.current?.render(sceneRef.current!, cam);
  ...
  const takeScreenshot = useCallback(() => {
    ...
    r.domElement.toBlob(blob => {
      ...
      a.href = url; a.download = `${school?.name || "school"}_3d.png`; a.click();
  ...
  const toggleGrid = (v: boolean) => {
    ...
    if (g) g.visible = v; setGridVisible(v);
  };
  ...
  const setTool = (t: ToolMode) => {
    setToolMode(t); setMeasurePoints([]); hoverRef.current = null; setPendingAnnotation(null);
  };

  // ── Render ────────────────────────────────────────────────────────────────
  return (
    <div className="fixed inset-0 bg-[#0f1117] flex flex-col" style={{ fontFamily: "system-ui, sans-serif" }}>

      <ViewerToolbar
        school={school}
        toolMode={toolMode} onSetTool={setTool}
        annotationColor={annotationColor} onSetAnnotationColor={setAnnotationColor}
        homePosition={homePosition} onGoHome={() => homePosition && goHome(homePosition)}
        savingHome={savingHome} homeSaved={homeSaved} onSaveHome={saveHomePosition}
        onScreenshot={takeScreenshot}
        showSettings={showSettings} onToggleSettings={() => setShowSettings(v => !v)}
        showHelp={showHelp} onToggleHelp={() => setShowHelp(v => !v)}
        movementPaused={movementPaused} annotationCount={annotations.length}
      />
      ...
      <canvas
        ...
        style={{ inset: 0, top: 44, width: "100%", height: "calc(100% - 44px)", zIndex: 18 }}
      />
      ...
      {annotations.map(ann => (
        <AnnotationLabel
          key={ann.id}
          ann={ann}
          ref={el => { annDomRefs.current[ann.id] = el; }}
          onEdit={() => { setEditingAnnotation(ann.id); setEditLabel(ann.label); }}
      ...
      {pendingAnnotation && (
        <NewAnnotationDialog
          ...
          label={annotationLabel} color={annotationColor}
          onLabelChange={setAnnotationLabel} onColorChange={setAnnotationColor}
          onConfirm={confirmAnnotation} onCancel={() => setPendingAnnotation(null)}
        />
      )}

      {editingAnnotation && (
        <EditAnnotationDialog
          label={editLabel} onLabelChange={setEditLabel}
          onConfirm={confirmEdit} onCancel={() => setEditingAnnotation(null)}
        />
      )}

      <MeasurementHUD
        distance={distance} area={area} perimeter={perimeter}
        unit={measureUnit} onSetUnit={setMeasureUnit}
        onClear={() => setMeasurePoints([])}
      />

      {toolMode !== "none" && !pendingAnnotation && (
        <div className="absolute top-14 left-1/2 -translate-x-1/2 z-20 px-4 py-2 rounded-xl text-xs pointer-events-none"
          style={{ background: "rgba(59,130,246,0.2)", color: "#93c5fd", border: "1px solid rgba(59,130,246,0.3)", backdropFilter: "blur(8px)" }}>
          {toolMode === "distance" && "Click two points on the model to measure distance"}
          {toolMode === "area" && "Click 3+ points for area — Clear when done"}
          {toolMode === "annotate" && "Click a point on the model to place an annotation"}
        </div>
      )}

      {showSettings && (
        <SettingsPanel
          gridVisible={gridVisible} onToggleGrid={toggleGrid}
          fogEnabled={fogEnabled} onToggleFog={toggleFog}
          unit={measureUnit} onSetUnit={setMeasureUnit}
      ...
      {savingAnnotations && (
        <div className="absolute bottom-12 right-4 z-20 px-3 py-1.5 rounded-lg text-xs"
      ...
      {/* Status bar */}
      <div className="absolute bottom-0 left-0 right-0 z-20 flex items-center justify-between px-5 py-2"
        style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(8px)", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
        <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 10, fontWeight: 700, textTransform: "uppercase" }}>
          {school?.district && `${school.district} · ${school.province}`}
        </span>
        <div className="flex items-center gap-3" style={{ color: "rgba(255,255,255,0.2)", fontSize: 10 }}>
          <span>WASD — Move</span><span>QE — Up/Down</span><span>Space — Pause</span><span>Scroll — Zoom</span>
        </div>
      </div>

      <LoadingOverlay school={school} loading={loading} loadProgress={loadProgress} error={error} />
    </div>
  );
}
