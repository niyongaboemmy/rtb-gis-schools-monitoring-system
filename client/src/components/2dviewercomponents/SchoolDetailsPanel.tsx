...
const statusColors: Record<string, string> = {
  ...
  under_renovation:
    "bg-amber-500/10 dark:bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-500/25",
};
...
function Section({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-linear-to-b from-slate-200/50 dark:from-gray-500/10 to-blue-500/0 p-px opacity-20 group-hover:opacity-40 transition-opacity">
        <div className="w-full h-full bg-white dark:bg-gray-900/60 backdrop-blur-3xl rounded-[calc(1.5rem-1px)]" />
      </div>
...
function KV({
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      <span
        className={cn(
          "text-[10px] text-slate-500 dark:text-white/40 font-normal tracking-wide leading-none",
          labelClassName,
        )}
      >
        {label}
      ...
      <span
        className={cn(
          "text-[14px] text-slate-800 dark:text-white/90 font-medium leading-relaxed wrap-break-word min-w-0 antialiased tracking-tight",
          valueClassName,
        )}
      >
        {value ?? "—"}
  ...
  return (
    <div
      className={cn(
        ...
        config.color,
        "bg-white dark:bg-transparent", // Light mode compatibility
      )}
    >
...
export const SchoolDetailsPanel: React.FC<SchoolDetailsPanelProps> = ({
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="h-full flex flex-col w-full bg-slate-50/50 dark:bg-transparent dark:backdrop-blur-3xl overflow-hidden"
    >
        ...
        <div className="absolute inset-0 z-0">
          {s.thumbnailUrl ? (
            <img
              src={s.thumbnailUrl}
              alt={s.name}
              className="w-full h-full object-cover blur-3xl opacity-10 dark:opacity-20 scale-110"
            />
          ) : (
            <div className="w-full h-full bg-linear-to-br from-primary/20 dark:from-primary/10 to-transparent blur-3xl opacity-30" />
          )}
        ...
        <div className="relative z-10 w-full mx-auto px-6 sm:px-10 py-6 sm:py-8 flex flex-col md:flex-row items-center md:items-end gap-6 sm:gap-10">
          ...
          <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left gap-5">
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5">
              ...
              {s.type && (
                <span className="text-[10px] font-normal tracking-wide px-3.5 py-1.5 rounded-xl bg-primary/10 text-primary/80 border-2 border-primary/20">
            ...
            <div className="space-y-3">
              <h1 className="text-xl sm:text-2xl lg:text-4xl font-medium text-slate-900 dark:text-white leading-none tracking-tight antialiased">
                {s.name}
              </h1>
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-slate-400 dark:text-white/30">
                <p className="text-sm font-mono font-medium tracking-wide text-slate-500 dark:text-white/50">
                  Registry id: {s.code}
                </p>
                {s.establishedYear && (
                  <>
                    <span className="h-1 w-1 rounded-full bg-slate-200 dark:bg-white/20" />
                    <p className="text-[11px] font-normal tracking-wide text-slate-400 dark:text-white/40">
                      Established {s.establishedYear}
                    </p>
                  </>
                )}
                <span className="h-1 w-1 rounded-full bg-slate-200 dark:bg-white/20" />
                <div className="flex items-center gap-1.5 text-[10px] font-normal tracking-wide text-slate-400 dark:text-white/30">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 md:mb-1">
            ...
            >
              ...
              <div className="relative flex items-center gap-2.5">
                <Pencil className="w-3.5 h-3.5 text-blue-500 dark:text-blue-400 group-hover:scale-110 transition-transform duration-300" />
                <span className="text-[11px] font-bold text-slate-600 dark:text-white/70 group-hover:text-slate-900 dark:group-hover:text-white transition-colors tracking-wider uppercase antialiased">
                  Edit Profile
                </span>
              </div>
            </button>
            ...
            >
              ...
              <div className="relative flex items-center gap-2.5">
                <ClipboardCheck className="w-3.5 h-3.5 text-cyan-500 dark:text-cyan-400 group-hover:scale-110 transition-transform duration-300" />
                <span className="text-[11px] font-bold text-slate-600 dark:text-white/70 group-hover:text-slate-900 dark:group-hover:text-white transition-colors tracking-wider uppercase antialiased">
                  Survey Update
                </span>
              </div>
      ...
      <div className="container mx-auto flex-1 overflow-y-auto px-4 sm:px-8 py-4 sm:py-6 custom-scrollbar">
        <motion.div
          ...
          className="container mx-auto space-y-8 pb-20"
        >
                  ...
                  <div className="p-6 rounded-2xl bg-slate-50 dark:bg-white/3 border border-slate-200 dark:border-blue-900/20 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[10px] font-normal text-slate-500 dark:text-white/50 tracking-wide">
                          Geospatial registry
                        </span>
                      </div>
                      <div
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                      ...
                      <KV
                        label="Longitude"
                        value={
                          s.longitude ? Number(s.longitude).toFixed(7) : "—"
                        }
                      />
                      <KV
                        label="Elevation (H)"
                        value={val(s.elevation, " m")}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  {s.headTeacher && (
                    <div className="p-3 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-blue-900/20 flex items-center gap-6 group/head hover:bg-white dark:hover:bg-white/8 transition-all duration-300">
                      ...
                      <div className="space-y-1">
                        <p className="text-xs font-normal text-slate-500 dark:text-white/50 tracking-wide">
                          Legal representative
                        </p>
                        <p className="text-base font-medium text-slate-800 dark:text-white/80 leading-none group-hover/head:text-primary dark:group-hover/head:text-white transition-colors">
                          {s.headTeacher}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                      <KV
                        ...
                        value={
                          <a
                            href={`tel:${s.phone}`}
                            className="flex items-center gap-2 text-slate-700 dark:text-white/90 hover:text-primary transition-colors"
                          >
                            <Phone className="w-3 h-3 text-slate-400 dark:text-white/20" />
                            {s.phone}
                          </a>
                        }
                      />
                      <KV
                        ...
                        value={
                          <a
                            ...
                            className="flex items-center gap-2 text-slate-700 dark:text-white/90 hover:text-primary transition-colors"
                          >
                            <Mail className="w-3 h-3 text-slate-400 dark:text-white/20" />
                    ...
                    <KV
                      ...
                      value={
                        <div className="flex items-center gap-2">
                          <Globe className="w-3 h-3 text-slate-400 dark:text-white/40" />
                ...
                <div className="space-y-8">
                  ...
                  <div className="p-6 rounded-2xl bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-blue-900/20 space-y-6">
                    <div className="flex items-center justify-between border-b border-slate-100 dark:border-blue-900/20 pb-4">
                      ...
                      <span className="text-xs font-medium text-primary dark:text-primary/80 tracking-tight">
              ...
              <div className="space-y-10">
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between px-1">
                    <div className="flex items-center gap-2.5">
                      <GraduationCap className="w-4 h-4 text-blue-500 dark:text-blue-400/60" />
                      <span className="text-xs font-normal text-slate-500 dark:text-white/50 tracking-wide font-mono">
                        Student population
                      </span>
                    </div>
                    <span className="text-lg font-medium text-slate-800 dark:text-white/80">
                      {val(s.totalStudents)}
                    </span>
                  </div>
                  <RegistryGrid cols={2} gap="gap-4">
                    ...
                    <div className="p-4 rounded-3xl bg-pink-50 dark:bg-pink-500/5 border border-pink-100 dark:border-pink-500/10 flex justify-between items-center">
                      <span className="text-[10px] font-normal text-pink-600 dark:text-pink-400/60 tracking-wide">
                        FEMALE
                      </span>
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between px-1">
                    <div className="flex items-center gap-2.5">
                      <Users className="w-4 h-4 text-amber-500 dark:text-amber-400/60" />
                      <span className="text-xs font-normal text-slate-500 dark:text-white/50 tracking-wide font-mono">
                        Academic staff
                      </span>
                    </div>
              ...
              {s.buildings && s.buildings.length > 0 ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] font-normal text-slate-400 dark:text-white/40 tracking-widest">
                      ...
                      >
                        <div className="flex justify-between items-start mb-3">
                          <div className="space-y-0.5">
                            <p className="text-sm font-medium text-slate-800 dark:text-white/80 group-hover/b:text-primary transition-colors">
                              {b.name}
                            </p>
                ...
                <div className="flex flex-col items-center justify-center py-20 gap-4 opacity-30">
                  <Building2 className="w-12 h-12 text-slate-300 dark:text-white/40" />
                  <p className="text-xs font-normal tracking-wide text-center text-slate-400 dark:text-white/50">
            ...
            >
              {loading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                  ...
                  <p className="text-[10px] font-normal text-slate-400 dark:text-white/40 tracking-wide">
                ...
                <div className="flex flex-col gap-8">
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                      <p className="text-[10px] font-normal text-slate-500 dark:text-white/40 tracking-wide mb-1">
                        Compliance matrix
                      </p>
                      <p className="text-sm font-normal text-slate-500 dark:text-white/40 tracking-wide">
                            ...
                            >
                              <div className="flex items-start justify-between gap-6">
                                <div className="space-y-1.5 flex-1">
                                  <p className="text-sm font-medium text-slate-700 dark:text-white/60 group-hover/item:text-slate-900 dark:group-hover/item:text-white transition-colors leading-tight">
                                    {item.label}
                                  </p>
                              ...
                              {(survey?.notes || survey?.inspectedAt) && (
                                <div className="space-y-3 pt-3 border-t border-slate-100 dark:border-gray-500/10">
                                  ...
                                  <div className="flex items-center justify-between opacity-30 group-hover/item:opacity-60 transition-opacity">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      <Calendar className="w-3 h-3 text-slate-400 dark:text-white/40" />
                                      <span className="text-[9px] font-normal tracking-wide text-slate-500 dark:text-white/40">
                                        {survey?.inspectedAt
                                          ? new Date(
                                              survey.inspectedAt,
                                            ).toLocaleDateString()
