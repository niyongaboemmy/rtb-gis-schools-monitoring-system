...
interface MetricBreakdown {
  label: string;
  /** null means the metric is not yet available — renders "—" instead of NaN% */
  score: number | null;
  icon: LucideIcon;
}
              ...
              <div className="mt-8 grid grid-cols-2 gap-4 w-full">
                ...
                <div className="p-3 rounded-2xl bg-background/40 border border-border/10 text-center">
                  ...
                  <p className="text-sm font-black text-primary uppercase leading-tight">
                    {isAggregate
                      ? (score < 50 ? "High" : "Normal")
                      : urgencyMonths != null
                        ? `${urgencyMonths} Months`
                        : "—"}
                  </p>
                </div>
              </div>
            ...
            <div className="lg:w-3/5 p-6 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {metrics.map((metric, index) => {
                  const safeScore = metric.score;
                  const hasScore  = safeScore != null && !isNaN(safeScore);
                  const barStatus = hasScore ? getStatus(safeScore) : null;
                    ...
                    >
                      ...
                      <div className="flex items-end justify-between gap-2">
                        ...
                        <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden mb-1">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: hasScore ? `${safeScore}%` : "0%" }}
                            transition={{ duration: 1, delay: 0.5 + index * 0.1 }}
                            className={cn(
                              "h-full rounded-full",
                              barStatus ? barStatus.bg : "bg-muted-foreground/20",
                            )}
                          />
                        </div>
                      </div>
