...
const MEASURE_COLORS = ["#6c47ff", "#ff4778", "#00d4aa", "#ffb347", "#7ec8e3", "#ff7f50"];
const ANNOT_COLORS = ["#00d4aa", "#ff4778", "#ffb347", "#7ec8e3", "#c4a8ff", "#ffffff"];
...
export default function GLBViewer() {
    ...
    // Persist
    useEffect(() => { saveJson(LS_MEASURES_KEY, measures); }, [measures]);
    useEffect(() => { saveJson(LS_ANNOTATIONS_KEY, annotations); }, [annotations]);
    ...
    const drawOverlay = useCallback(() => {
        ...
        if (measureMode && measureMode !== "annotate") {
            ...
            if (pendingPts.length > 0) {
                if (pendingPts.length >= 2) drawLine(pendingPts, col, false);

                if (measureMode === "area") {
                    const all = hoverPt ? [...pendingPts, hoverPt] : pendingPts;
                    drawPolygon(all, col, false);
                }

                if (hoverPt) {
                    const lastPt = pendingPts[pendingPts.length - 1];
                    drawLine([lastPt, hoverPt], col, true);
                    if (measureMode !== "distance" && pendingPts.length >= 2) {
                        drawLine([pendingPts[0], hoverPt], col, true);
                    }
                }

                pendingPts.forEach(p => drawDot(p, col, 5));

                if (hoverPt) {
                    let liveLabel = "";
                    if (measureMode === "area") {
                        const all = [...pendingPts, hoverPt]; liveLabel = fmtArea(calcPolyArea(all), unit);
                    } else {
                        let currentDist = calcDistance(pendingPts);
                        const dToHover = new THREE.Vector3(pendingPts[pendingPts.length - 1].x, pendingPts[pendingPts.length - 1].y, pendingPts[pendingPts.length - 1].z)
                            .distanceTo(new THREE.Vector3(hoverPt.x, hoverPt.y, hoverPt.z));
                        let total = currentDist + dToHover;
                        if (measureMode === "perimeter" && pendingPts.length >= 2) {
                            const dBackToStart = new THREE.Vector3(pendingPts[0].x, pendingPts[0].y, pendingPts[0].z).distanceTo(new THREE.Vector3(hoverPt.x, hoverPt.y, hoverPt.z));
                            total += dBackToStart;
                        }
        ...
        if (visibility === "all" || visibility === "annotations") {
            annotations.forEach(a => {
                ...
                ctx.fillStyle = "rgba(7,7,13,0.88)";
    ...
    }, [measures, pendingPts, measureMode, unit, annotations, annotPendingPt, visibility, annotColor]);
    ...
    const getFastHoverRaycastPt = useCallback((e: React.MouseEvent<HTMLCanvasElement>): MeasurePoint | null => {
        ...
        if (pendingPts.length > 0) {
            const last = pendingPts[pendingPts.length - 1]; referencePt.set(last.x, last.y, last.z);
    ...
    const handleOverlayClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        setPendingPts(prev => {
            if (prev.length > 0) {
                ...
                // 🔥 CRITICAL FIX: Changed from 0.001 to 0.000001 so small scale maps don't reject perfectly valid secondary points!
                if (dist < 0.000001) return prev;
            }
            return [...prev, pt];
        });
    ...
    const handleScreenshot = useCallback(() => {
        ...
        const a = document.createElement("a"); a.href = url; a.download = `glb_viewer_${Date.now()}.png`; a.click();
    ...
    const loadGLB = useCallback(async (file: File) => {
        ...
        try {
            const arrayBuffer = await new Promise<ArrayBuffer>((resolve, reject) => {
                ...
                reader.onprogress = e => { if (e.lengthComputable) { setProgress(Math.round(e.loaded / e.total * 40)); setProgressLabel(`Reading… ${formatBytes(e.loaded)} / ${formatBytes(e.total)}`); } };
                reader.onload = () => resolve(reader.result as ArrayBuffer);
                reader.onerror = () => reject(new Error("File read failed"));
                reader.readAsArrayBuffer(file);
            });
            setProgress(42); setProgressLabel("Initialising loaders…"); setPhase("viewing");
            await new Promise(r => setTimeout(r, 80));
            setProgress(45); setProgressLabel("Parsing GLB…");

            const dracoLoader = new DRACOLoader(); dracoLoader.setDecoderPath("https://www.gstatic.com/draco/versioned/decoders/1.5.6/");
            ...
            setProgress(90); setProgressLabel("Building scene…");
            ...
            scene.add(model); modelRef.current = model; applyRenderMode(renderMode);
            ...
            const distance = (halfDiag / Math.tan(fovRad / 2)) * 1.2;
            camera.near = distance * 0.0001; camera.far = distance * 100; camera.updateProjectionMatrix();

            let savedHome: CameraHome | null = null;
            try { const s = localStorage.getItem(getHomeKey(file.name)); if (s) savedHome = JSON.parse(s); } catch { }
            if (!savedHome) { try { const s = localStorage.getItem(LS_HOME_KEY); if (s) savedHome = JSON.parse(s); } catch { } }
            ...
            if (savedHome) {
                ...
                const hp = new THREE.Vector3(center.x + distance * 0.55, center.y + distance * 0.35, center.z + distance * 0.55);
            ...
            const newGrid = new THREE.GridHelper(gridSize, divisions, 0x1a1a2e, 0x1a1a2e);
            (newGrid.material as THREE.Material).opacity = 0.4; (newGrid.material as THREE.Material).transparent = true; newGrid.position.y = box.min.y; scene.add(newGrid);
            ...
            setStats({ fileName: file.name, fileSize: file.size, loadTime: Date.now() - startTime, triangles: Math.round(triangles), meshes: meshCount, textures: textureCount });
    ...
    return (
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .glb-root{width:100vw;height:100vh;background:#07070d;display:flex;flex-direction:column;font-family:'Syne',sans-serif;color:#e8e8f0;overflow:hidden}
        .header{display:flex;align-items:center;justify-content:space-between;padding:14px 24px;border-bottom:1px solid rgba(255,255,255,0.06);background:rgba(7,7,13,0.95);backdrop-filter:blur(12px);z-index:10;flex-shrink:0}
        .logo{display:flex;align-items:center;gap:10px;font-size:17px;font-weight:800;letter-spacing:-0.5px}
        .logo-icon{width:30px;height:30px;border-radius:8px;background:linear-gradient(135deg,#6c47ff,#ff4778);display:flex;align-items:center;justify-content:center;font-size:14px}
        .badge{font-family:'JetBrains Mono',monospace;font-size:10px;padding:3px 8px;background:rgba(108,71,255,0.15);border:1px solid rgba(108,71,255,0.35);border-radius:20px;color:#9d7aff;letter-spacing:0.5px}
        ...
        .drop-zone::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 60% 40% at 50% 50%,rgba(108,71,255,0.08) 0%,transparent 70%),radial-gradient(ellipse 40% 30% at 20% 80%,rgba(255,71,120,0.05) 0%,transparent 60%);pointer-events:none}
        .drop-target{width:100%;max-width:520px;aspect-ratio:4/3;border:2px dashed rgba(108,71,255,0.35);border-radius:20px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;cursor:pointer;transition:all .25s ease;background:rgba(108,71,255,0.03);position:relative;overflow:hidden}
        .drop-target.dragging{border-color:rgba(108,71,255,0.8);background:rgba(108,71,255,0.08);transform:scale(1.01)}
        .drop-icon{width:72px;height:72px;border-radius:18px;background:linear-gradient(135deg,rgba(108,71,255,0.2),rgba(255,71,120,0.1));border:1px solid rgba(108,71,255,0.3);display:flex;align-items:center;justify-content:center;font-size:30px;transition:transform .25s ease}
        ...
        .upload-btn{padding:11px 26px;border-radius:10px;border:none;background:linear-gradient(135deg,#6c47ff,#9d47ff);color:#fff;font-family:'Syne',sans-serif;font-size:13px;font-weight:700;cursor:pointer;transition:all .2s ease}
        .upload-btn:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(108,71,255,0.4)}
        ...
        .info-card-value{font-family:'JetBrains Mono',monospace;font-size:15px;font-weight:500;color:#9d7aff}
        ...
        .loading-overlay::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 50% 40% at 50% 50%,rgba(108,71,255,0.1) 0%,transparent 70%);pointer-events:none}
        .spinner-ring{width:72px;height:72px;border-radius:50%;border:3px solid rgba(108,71,255,0.15);border-top:3px solid #6c47ff;border-right:3px solid #ff4778;animation:spin 1s linear infinite}
        @keyframes spin{to{transform:rotate(360deg)}}
        .loading-pct{font-size:44px;font-weight:800;letter-spacing:-2px;background:linear-gradient(135deg,#6c47ff,#ff4778);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
        .progress-track{width:300px;height:3px;background:rgba(255,255,255,0.07);border-radius:4px;overflow:hidden}
        .progress-fill{height:100%;background:linear-gradient(90deg,#6c47ff,#ff4778);border-radius:4px;transition:width .3s ease}
        ...
        .tb-group{display:flex;flex-direction:column;gap:3px;background:rgba(7,7,13,0.88);backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,0.09);border-radius:12px;padding:5px}
        ...
        .tb-btn:hover{background:rgba(108,71,255,0.18);color:#c4a8ff}
        .tb-btn.active{background:rgba(108,71,255,0.3);color:#c4a8ff}
        ...
        .tb-btn[data-tip]:hover::after{content:attr(data-tip);position:absolute;left:calc(100% + 10px);top:50%;transform:translateY(-50%);white-space:nowrap;background:rgba(7,7,13,0.97);border:1px solid rgba(255,255,255,0.1);border-radius:7px;padding:5px 10px;font-family:'JetBrains Mono',monospace;font-size:10px;color:#e8e8f0;letter-spacing:0.3px;pointer-events:none;z-index:100}
        .speed-group{display:flex;flex-direction:column;gap:0;background:rgba(7,7,13,0.88);backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,0.09);border-radius:12px;padding:5px;align-items:center}
        ...
        .speed-val{font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:600;color:#9d7aff;padding:3px 0;text-align:center;min-width:36px;letter-spacing:-0.3px}
        .speed-track{width:26px;height:3px;background:rgba(255,255,255,0.07);border-radius:3px;overflow:hidden;margin:1px 0 3px}
        .speed-fill{height:100%;background:linear-gradient(90deg,#6c47ff,#ff4778);border-radius:3px;transition:width .15s ease}
        ...
        .mode-btn{padding:7px 12px;border-radius:8px;border:1px solid rgba(255,255,255,0.08);background:rgba(7,7,13,0.88);backdrop-filter:blur(16px);color:rgba(232,232,240,0.45);font-family:'JetBrains Mono',monospace;font-size:10px;cursor:pointer;transition:all .18s ease;text-align:left;display:flex;align-items:center;gap:7px;white-space:nowrap}
        .mode-btn:hover{background:rgba(108,71,255,0.12);color:#c4a8ff;border-color:rgba(108,71,255,0.3)}
        .mode-btn.active{background:rgba(108,71,255,0.2);color:#c4a8ff;border-color:rgba(108,71,255,0.5)}
        .mode-dot{width:7px;height:7px;border-radius:50%;background:rgba(255,255,255,0.15);flex-shrink:0}
        .mode-btn.active .mode-dot{background:#9d7aff}
        ...
        .stats-panel{position:absolute;bottom:16px;left:16px;background:rgba(7,7,13,0.88);backdrop-filter:blur(16px);border:1px solid rgba(255,255,255,0.09);border-radius:14px;padding:12px 16px;display:flex;gap:16px;align-items:center;z-index:20}
        .stat-item{display:flex;flex-direction:column;gap:2px}
        .stat-val{font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:500;color:#9d7aff}
        ...
        /* Measure toolbar */
        .meas-toolbar{position:absolute;bottom:16px;left:50%;transform:translateX(-50%);display:flex;align-items:center;gap:6px;background:rgba(7,7,13,0.92);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:8px 12px;z-index:25;transition:all .25s ease}
        .meas-toolbar.active-mode{border-color:rgba(108,71,255,0.5);box-shadow:0 0 0 1px rgba(108,71,255,0.2),0 8px 32px rgba(0,0,0,0.5)}
        ...
        .meas-btn:hover{background:rgba(108,71,255,0.12);color:#c4a8ff;border-color:rgba(108,71,255,0.25)}
        .meas-btn.active{background:rgba(108,71,255,0.22);color:#c4a8ff;border-color:rgba(108,71,255,0.5)}
        ...
        .meas-hint{font-family:'JetBrains Mono',monospace;font-size:9px;color:rgba(108,71,255,0.7);padding:0 4px;white-space:nowrap}
        ...
        .unit-btn:hover{background:rgba(108,71,255,0.1);border-color:rgba(108,71,255,0.3);color:#c4a8ff}
        .unit-dropdown{position:absolute;bottom:calc(100% + 6px);left:0;min-width:170px;background:#0c0c18;border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:5px;box-shadow:0 16px 48px rgba(0,0,0,0.7);z-index:100;animation:dropUp .15s ease}
        ...
        .unit-opt:hover{background:rgba(108,71,255,0.12);color:#c4a8ff}
        .unit-opt.selected{color:#9d7aff;background:rgba(108,71,255,0.1)}
        .unit-opt .check{color:#9d7aff;font-size:12px}

        /* Measure panel (right drawer) */
        .meas-panel{position:absolute;right:16px;top:80px;bottom:80px;width:280px;background:rgba(7,7,13,0.94);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.09);border-radius:16px;z-index:20;display:flex;flex-direction:column;overflow:hidden;animation:panelIn .2s ease}
        ...
        .mp-count{font-family:'JetBrains Mono',monospace;font-size:9px;padding:2px 7px;background:rgba(108,71,255,0.15);border-radius:20px;color:#9d7aff}
        ...
        .mp-list::-webkit-scrollbar-thumb{background:rgba(108,71,255,0.3);border-radius:2px}
        ...
        .annot-dialog{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:#0c0c18;border:1px solid rgba(255,255,255,0.12);border-radius:16px;padding:20px;width:300px;z-index:60;box-shadow:0 32px 80px rgba(0,0,0,0.8);animation:popIn .05s ease-out forwards}
        ...
        .ad-textarea:focus{border-color:rgba(108,71,255,0.5)}
        ...
        .ad-save{padding:7px 18px;border-radius:8px;border:none;background:linear-gradient(135deg,#6c47ff,#9d47ff);color:#fff;font-family:'JetBrains Mono',monospace;font-size:10px;font-weight:600;cursor:pointer;transition:all .15s}
        .ad-save:hover{transform:translateY(-1px);box-shadow:0 4px 16px rgba(108,71,255,0.4)}
        ...
        .help-panel{width:580px;max-width:94vw;max-height:90vh;overflow-y:auto;background:#0c0c18;border:1px solid rgba(255,255,255,0.1);border-radius:20px;box-shadow:0 40px 100px rgba(0,0,0,0.8);animation:bslideUp .22s ease}
        .help-header{display:flex;align-items:center;justify-content:space-between;padding:18px 22px 16px;border-bottom:1px solid rgba(255,255,255,0.07);position:sticky;top:0;background:#0c0c18;z-index:1;border-radius:20px 20px 0 0}
        .help-title{font-size:15px;font-weight:800;letter-spacing:-0.3px;display:flex;align-items:center;gap:10px}
        .help-title-icon{width:28px;height:28px;border-radius:8px;background:linear-gradient(135deg,#6c47ff,#ff4778);display:flex;align-items:center;justify-content:center;font-size:13px}
        ...
        .help-tip{padding:12px 14px;border-radius:10px;background:rgba(108,71,255,0.07);border:1px solid rgba(108,71,255,0.2);font-family:'JetBrains Mono',monospace;font-size:10px;color:rgba(157,122,255,0.8);line-height:1.8}
      ...
      `}</style>
            ...
            <div className="glb-root">
                <header className="header">
                    <div className="logo"><div className="logo-icon">⬡</div> RTB Drone Map Viewer</div>
                ...
                {phase === "idle" && (
                    <div className="drop-zone">
                        <label className={`drop-target${isDragging ? " dragging" : ""}`}
                            ...
                            <div className="drop-title">Drop your .glb model here</div>
                            <div className="drop-sub">Measure distances · Areas · Perimeters<br />Annotations · Screenshot · Unlit photogrammetry</div>
                ...
                {phase === "viewing" && (
                    <div className="viewer-wrapper">
                        {progress < 100 && (
                            <div style={{ position: "absolute", inset: 0, zIndex: 5, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 24, background: "rgba(7,7,13,0.78)", backdropFilter: "blur(6px)" }}>
                                <div className="spinner-ring" />
                                <div className="loading-pct">{progress}%</div>
                                <div className="progress-track"><div className="progress-fill" style={{ width: `${progress}%` }} /></div>
                                <div className="progress-label">{progressLabel}</div>
                        ...
                        <div className="toolbar">
                            <div className="tb-group">
                                <button className="tb-btn" data-tip="Load new file"
                                    onClick={() => { setPhase("idle"); setProgress(0); setStats(null); setError(null); setMeasureMode(null); setPendingPts([]); hoverPtRef.current = null; }}>⬡</button>
